...
if st.session_state.items:
    ...
    with c2:
        ...
        if st.button("Eliminar", type="secondary"):
            if 0 < idx_del <= len(st.session_state.items):
                st.session_state.items.pop(idx_del-1)

# TOTALES
_total_neto = sum(i["cantidad"] * i["precioUnitario"] for i in st.session_state.items)
_total_itbms = sum(i["valorITBMS"] for i in st.session_state.items)
